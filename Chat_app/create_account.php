<?php
    ...
    if(isset($_POST["email"])){
        ...
        if((filter_var($email_san, FILTER_VALIDATE_EMAIL)==false) || ($email_san!=$email)){
            $validation = false;
            $_SESSION['err_email'] = '<span class="error">Podaj poprawny adres email</span>';
        }
        ...
        if(ctype_alnum($nick)==false){
            $validation = false;
            $_SESSION['err_nick'] = '<span class="error">Nick może składać się tylko z liter i cyfr</span>';
        }
        ...
        if($password != $confirm_password){
            $validation = false;
            $_SESSION['err_password'] = '<span class="error">Hasła nie są identyczne</span>';
        }

        if(strlen($password) < 8 || strlen($password) > 30){
            $validation = false;
            $_SESSION['err_password'] = '<span class="error">Hasło musi posiadać od 8 do 30 znaków</span>';
        }
        ...
        try{
            ...
            if($conn->connect_errno!=0){
                ...
                if($sql->num_rows > 0){
                    $validation = false;
                    $_SESSION["err_email"] = '<span class="error">Zajęty jest już taki nick</span>';
                }

                if($validation == true){
                    $sql = $conn->query("INSERT INTO users VALUES(NULL, '$nick', '$email', '$password_hash', now())");
                    if(!$sql){
                        throw new Exception($conn->error);
                    }else{
